...
func (r *Repository) UpdateSoulBalanceOf(contract string, tokenID string, balanceOf float64) (*mongo.UpdateResult, error) {
	f := bson.D{
		{"collection_address", contract},
		{"token_id", tokenID},
	}

	update := bson.M{"$set": bson.M{"soul_balance_of": balanceOf}}
