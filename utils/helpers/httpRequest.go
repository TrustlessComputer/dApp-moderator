...
func HttpRequest(fullUrl string, method string, headers map[string]string, reqBody interface{}) ([]byte, int, error) {
	...
	if !isAllowed {
		...
		if ok {
			return nil, res.StatusCode, errors.New(dataErrorString)
		}
		...
		if err != nil {
			if data.Message != nil {
				return nil, res.StatusCode, errors.New(*data.Message)
			}
		}
		return nil, res.StatusCode, errors.New(dataError.Message)
