...
func startServer() {
	...
	if err != nil {
		logger.AtLog.Logger.Error("CreateCollectionIndexes - Cannot created index ", zap.Error(err))
		// return
	}
	...
	if migrateErr := migrate.Up(-1); migrateErr != nil {
		logger.AtLog.Errorf("migrate failed", zap.Error(err))
	}

	uc, err := usecase.NewUsecase(&g, *repo)
	if err != nil {
		logger.AtLog.Errorf("LoadUsecases - Cannot init usecase", zap.Error(err))
		return
	}
